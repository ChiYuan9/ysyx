...
#define immB() do { *imm = (SEXT(BITS(i, 31, 31), 1) << 12) | (BITS(i, 7, 7) << 11) | (BITS(i, 30, 25) << 5) | (BITS(i, 11, 8) << 1) | 0x0;} while(0)
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT_START();
  INSTPAT("? ?????????? ? ???????? ????? 11011 11", jal    , J, R(rd) = s->pc + 4; s->dnpc = s->pc + imm);  // dummy.c
  INSTPAT("???????????? ????? 000 ????? 00100 11" , addi   , I, R(rd) = src1 + imm);
  INSTPAT("???????????? ????? 000 ????? 11001 11" , jalr   , I, R(rd) = s->pc + 4; s->dnpc = (src1 + imm) & 0xfffffffe);
  INSTPAT("???????????? ????? 010 ????? 00000 11" , lw     , I, R(rd) = SEXT(Mr(src1 + imm, 4), 32)); // add.c
  ...
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , R, R(rd) = src1 + src2);  // add-longlong.c
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + imm, 2, src2));  // bit.c
  INSTPAT("010000 ?????? ????? 101 ????? 00100 11", srai   , I, R(rd) = SEXT(BITS(src1, 63, 63), 1) | (src1 >> BITS(imm, 5, 0)));
